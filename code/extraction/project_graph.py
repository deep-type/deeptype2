...
from wikidata_linker_utils.entrypoint import boolean_argument
...
def parse_args():
    ...
    boolean_argument(parser, "interactive", default=True)
    boolean_argument(parser, "use-cache", default=True)
...
def export_classification(classification, path):
    ...
    with open(join(path, "classes.txt"), "wt", encoding="utf-8") as fout:
